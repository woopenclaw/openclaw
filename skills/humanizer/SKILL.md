...
识别并移除 AI 生成文本的典型痕迹，让文字读起来像真人写的。基于 Wikipedia "Signs of AI writing" 指南（由 WikiProject AI Cleanup 维护），源自对数千个 AI 生成文本实例的观察。覆盖 AI 常用句式、过度正式化、模式化连接词、空泛修饰语等反模式。

触发条件：任何需要让文本「去 AI 化」的场景——博客文章、邮件、报告、社交媒体帖子、代码注释。不是翻译或重写工具——是在保留原意的前提下消除 AI 痕迹。
...
> 核心操作与代理指令速查。29 种 AI 文本模式的快速入口。详细说明见下方 Pattern 段和 Process 段。

| 操作 | 指令 / 入口 | 位置 |
|------|------------|------|
| **输入加载** | `read_file(path)` 加载待人类化文本，扫描 29 种 AI 模式 | §Process L490 |
| **语音校准** | 6 维分析样本：句长/选词/段首/标点/口头禅/过渡词 | §Layer 2 (Pattern 30) L64-72 |
| **个性注入** | 5 层：观点/节奏变异/第一人称/混乱/具体感受 | §Soul L95-105 |
| **自我审计** | "What makes the below so obviously AI generated?" — 产出审计清单 | §Process L500 |
| **修复通道** | "Now make it not obviously AI generated." — 基于审计清单改写 | §Process L502 |
| **工具派发** | `patch(path, old, new)` 手术级编辑 / `write_file(path, content)` 完全重写 | §Process L504 |
| **交付格式** | Draft → Audit bullets → Final → Change summary（4 段式产出） | §Process L508-512 |

**关键入口文件**: `references/29-patterns-reference.md`（29 种 AI 模式表格）· `references/voice-calibration.md`（语音匹配指南）
**防踩坑**: 自我审计必须在交付前执行 · `patch` 用于小编辑（<100 行），`write_file` 用于大改写 · 不要跳过 6 维语音分析直接改
